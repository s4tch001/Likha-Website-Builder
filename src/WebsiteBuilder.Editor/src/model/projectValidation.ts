...
export function isSafeCssValue(value: string): boolean {
  if (
    value.length > 8_192 ||
    /[\0\r\n{};]/.test(value) ||
    /@import|expression\(|javascript:|vbscript:|-moz-binding|behavior:/i.test(value)
  ) {
    return false;
  }
...
function isSafeAttribute(name: string, value: string): boolean {
  ...
  try {
    ...
    return url.protocol === "https:" || url.protocol === "http:" ||
      (/^href$/i.test(name) && (url.protocol === "mailto:" || url.protocol === "tel:"));
  } catch {
...
function validateStyleRecord(value: unknown): value is Record<string, string> {
  return isStringRecord(value) && Object.entries(value).every(
...
function validateNodeShape(value: unknown): value is ElementNode {
  ...
  return (
    ...
    Array.isArray(value.children) && value.children.length <= 10_000 &&
    isStringRecord(value.attributes) && Object.entries(value.attributes).every(
      ([name, entry]) => isSafeAttribute(name, entry),
    ) &&
    validateStyleRecord(value.styles) &&
    isRecord(value.responsiveStyles) && Object.values(value.responsiveStyles).every(validateStyleRecord)
  );
}
...
export function isValidProject(value: unknown): value is Project {
  if (!isRecord(value) || value.schemaVersion !== 2 || !Array.isArray(value.pages) || value.pages.length < 1) {
    return false;
  }
  if (!Array.isArray(value.breakpoints) || !Array.isArray(value.assets) || !isStringRecord(value.variables)) {
    return false;
  }
  if (typeof value.id !== "string" || typeof value.name !== "string" || value.pages.length > 1_000) {
    return false;
  }
  if (!Object.entries(value.variables).every(([name, entry]) =>
    isSafeCssPropertyName(name.startsWith("--") ? name : `--${name}`) && isSafeCssValue(entry))) {
    return false;
  }
  const breakpointIds = new Set<string>();
  let baseBreakpoints = 0;
  for (const breakpoint of value.breakpoints) {
    if (!isRecord(breakpoint) || typeof breakpoint.id !== "string" || typeof breakpoint.label !== "string" ||
        typeof breakpoint.maxWidth !== "number" || typeof breakpoint.isBase !== "boolean" ||
  ...
  if (baseBreakpoints !== 1 || value.assets.length > 10_000 || !value.assets.every((asset) =>
    isRecord(asset) && typeof asset.id === "string" && typeof asset.name === "string" &&
    typeof asset.storedFileName === "string" && typeof asset.relativePath === "string" &&
    asset.relativePath === `Assets/${asset.storedFileName}` && typeof asset.sha256 === "string" &&
  ...
  for (const page of value.pages) {
    if (!isRecord(page) || typeof page.id !== "string" || typeof page.route !== "string" || !isRecord(page.root)) {
      return false;
    }
    const route = page.route.replace(/^\/+|\/+$/g, "");
    if (pageIds.has(page.id) || routes.has(route.toLowerCase()) || !route.split("/").every((part) => routeSegment.test(part))) {
      return false;
    }
    pageIds.add(page.id);
    routes.add(route.toLowerCase());

    const stack: Array<{ node: unknown; depth: number }> = [{ node: page.root, depth: 0 }];
    while (stack.length > 0) {
      const current = stack.pop();
      if (!current || current.depth > MAX_DEPTH || !validateNodeShape(current.node) || ++count > MAX_ELEMENTS) {
        return false;
      }
