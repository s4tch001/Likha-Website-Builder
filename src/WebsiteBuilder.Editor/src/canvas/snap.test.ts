...
function node(id: string, x: number, y: number, w: number, h: number, children: ElementNode[] = []): ElementNode {
  return {
    id,
    type: "Div",
    x,
...
describe("computeSnap", () => {
  it("snaps a left edge to another element's left edge within threshold", () => {
    ...
    const proj = project([node("a", 100, 0, 50, 50), node("b", 140, 0, 50, 50)]);
    const drag = { origAbsX: 140, origAbsY: 0, origW: 50, origH: 50, exclude: subtreeIds(proj, "b") };
  ...
  it("does not snap when outside the threshold", () => {
    ...
    const proj = project([node("a", 600, 600, 50, 50), node("b", 140, 300, 50, 50)]);
    const drag = { origAbsX: 140, origAbsY: 300, origW: 50, origH: 50, exclude: subtreeIds(proj, "b") };
  ...
  it("excludes the dragged element's own subtree from snap targets", () => {
    ...
    const drag = { origAbsX: 200, origAbsY: 200, origW: 100, origH: 100, exclude: subtreeIds(proj, "parent") };
